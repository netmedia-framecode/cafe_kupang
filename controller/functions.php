    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $to       = $data['email'];
          $subject  = "Account Verification - Cafe Kupang";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di Cafe Kupang.</p>
  ...
  function forgot_password($conn, $data, $action, $baseURL)
  {
    if ($action == "update") {
      ...
      if (mysqli_num_rows($checkEmail) === 0) {
        ...
        require_once("mail.php");
        $to       = $data['email'];
        $subject  = "Reset password - Cafe Kupang";
        $message  = "<!doctype html>
        <html>
          <head>
              <meta name='viewport' content='width=device-width'>
  ...
  function login($conn, $data)
  {
    ...
    if (mysqli_num_rows($checkAccount) == 0) {
      ...
      if (password_verify($data['password'], $row["password"])) {
        $_SESSION["project_cafe_kupang"]["users"] = [
          "id" => $row["id_user"],
          "id_role" => $row["id_role"],
          "role" => $row["role"],
          "email" => $row["email"],
...
if (isset($_SESSION["project_cafe_kupang"]["users"])) {
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["project_cafe_kupang"]["name_page"] = "' . $data['title'] . '";
  ...
  function kafe($conn, $data, $action, $baseURL, $id_user)
  {
    if ($action == "insert") {
      $path = "../assets/img/kafe/";
      ...
      if (in_array($fileType, $allowTypes)) {
        ...
        compressImage($imageTemp, $imageUploadPath, 75);
        $url_image = $baseURL . "assets/img/kafe/" . $fileName_encrypt . "." . $ekstensiGambar;
      } else {
        $message = "Sorry, only JPG, JPEG and PNG image files are allowed.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
    ...
    if ($action == "update") {
      if (!empty($_FILES['avatar']["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
          $imageTemp = $_FILES["avatar"]["tmp_name"];
          compressImage($imageTemp, $imageUploadPath, 75);
          $unwanted_characters = $baseURL . "assets/img/kafe/";
          $remove_avatar = str_replace($unwanted_characters, "", $data['avatarOld']);
    ...
    if ($action == "delete") {
      ...
      $remove_avatar = str_replace($unwanted_characters, "", $data['avatarOld']);
      unlink($path . $remove_avatar);
      $sql = "DELETE FROM kafe WHERE id_kafe='$data[id_kafe]'";
    }
  ...
  function kriteria($conn, $data, $action)
  {
    if ($action == "insert") {
      ...
      $checkNama = mysqli_query($conn, "SELECT * FROM kriteria WHERE nama_kriteria='$data[nama]'");
      if (mysqli_num_rows($checkNama) > 0) {
        $message = "Maaf, nama kriteria sudah ada. Silakan masukan nama kriteria yang lain.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "INSERT INTO kriteria(kode_kriteria,nama_kriteria,bobot) VALUES('$kode_kriteria','$data[nama]','$data[bobot]')";
    }
    ...
    if ($action == "update") {
      ...
      $sql = "UPDATE kriteria SET nama_kriteria='$data[nama]', bobot='$data[bobot]', updated_at=current_timestamp WHERE id_kriteria='$data[id_kriteria]'";
    }
  ...
  {
    if ($action == "insert") {
      $sql = "INSERT INTO sub_kriteria(id_kriteria,sub_kriteria,nilai_sub) VALUES('$data[id_kriteria]','$data[sub_kriteria]','$data[nilai]')";
    }
  ...
  function alternatif($conn, $data, $action)
  {
    if ($action == "insert") {
      ...
      $checkKafe = mysqli_query($conn, "SELECT * FROM alternatif WHERE id_kafe='$data[id_kafe]'");
      if (mysqli_num_rows($checkKafe) > 0) {
        $message = "Maaf, alternatif sudah ada. Silakan pilih kafe yang lain sebagai alternatif.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "INSERT INTO alternatif(id_alternatif,kode_alternatif,id_kafe) VALUES('$id_alternatif','$kode_alternatif','$data[id_kafe]')";
      mysqli_query($conn, $sql);
    ...
    if ($action == "update") {
      $checkKafe = mysqli_query($conn, "SELECT * FROM alternatif WHERE id_kafe='$data[id_kafe]'");
      if (mysqli_num_rows($checkKafe) > 0) {
        $message = "Maaf, kafe sudah ada. Silakan pilih kafe yang lain sebagai alternatif.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "UPDATE alternatif SET id_kafe='$data[id_kafe]', updated_at=CURRENT_TIMESTAMP WHERE id_alternatif='$data[id_alternatif]'";
      mysqli_query($conn, $sql);
    }

    if ($action == "delete") {
      $sql = "DELETE FROM alternatif WHERE id_alternatif='$data[id_alternatif]'";
      mysqli_query($conn, $sql);
    }
  ...
  function nilai_alternatif($conn, $data, $action)
  {
    if ($action == "update") {
      if (empty($data['nilai'])) {
        $message = "Maaf, kamu belum menghitung total penilaian.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      if ($data['nilai'] > 100) {
        $message = "Maaf, penilaian yang kamu masukan telah melebihi batas maksimum yaitu 100.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "UPDATE nilai_alternatif SET nilai='$data[nilai]' WHERE id_nilai_alternatif='$data[id_nilai_alternatif]'";
    }
  ...
  function pemilihan($conn, $data, $action)
  {
    if ($action == "insert") {
      $total = $data['total'];
      $nama_kafe = $data['nama_kafe'];
      $count = count($nama_kafe);
      for ($x = 0; $x < $count; $x++) {
        if ($total[$x] >= 75) {
          $sql = "UPDATE kafe SET id_status='3', updated_at=CURRENT_TIMESTAMP WHERE nama_kafe='$nama_kafe[$x]'";
          mysqli_query($conn, $sql);
        } else if ($total[$x] < 75) {
          $sql = "UPDATE kafe SET id_status='2', updated_at=CURRENT_TIMESTAMP WHERE nama_kafe='$nama_kafe[$x]'";
          mysqli_query($conn, $sql);
        }
      }
    }
